...
try:
    from multiarea_model.default_params_schmidt import network_params, nested_update
    ...
    from default_params_schmidt import network_params, nested_update
...
def compute_Model_params(out_label='', mode='default'):
    ...
    process_raw_data()
    raw_fn = os.path.join(basepath, 'viscortex_raw_data-schmidt.json')
    proc_fn = os.path.join(basepath, 'viscortex_processed_data-schmidt.json')
    ...
    def num_CC_synapses(target_area, target_pop, source_area, source_pop):
        ...
        if target_area != source_area:
            print("target_area!=source_area")
        print(target_area, target_pop, source_area, source_pop, Nsyn)
        return Nsyn
    ...
    for target_area, target_pop, source_area, source_pop in product(area_list, population_list,
        ...
        if target_area != source_area:
            ...
            if N != 0:
                print("num_CC=",num_CC_synapses(target_area, target_pop,source_area, source_pop))
