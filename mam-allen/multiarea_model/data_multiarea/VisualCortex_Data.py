...
9. SLN data about connections to two areas (V1,V4) from
   Barone et al. (2000)
   ----> SLN_Data
10. Intrinsic connection probabilities from Billeh et al. (2020)
   ----> intrinsic_connectivity
11. Layer-specific number of neurons for cat V1 from Billeh et al.
    (2020) constructed from Binzegger et al. (2004)
    ----> Num_V1
...
def process_raw_data():
    ...
    """
    with open(os.path.join(datapath, 'Intrinsic_Connectivity_Allen.csv'), 'rt') as f:
        myreader = csv.reader(f, delimiter=',')
        skip_header()
        intrinsic_connectivity = {}
    ...
    """
    n = 17
    with open(os.path.join(datapath, 'Numbers_V1_Allen.csv'), 'rt') as f:
        myreader = csv.reader(f, delimiter=',')
        skip_header()
        num_V1 = {}
    ...
    """
    12.Two alternatives for determining laminar thicknesses:
    """
    ...
    We assume a laminar-specific ratio of each type of neurons to be constant across areas.
    """
    """
    Then, we compute the number of neurons in
    population i in layer v_i of area A as
    N(A,i) = rho(A,v_i) * S(A) * D(A,v_i) * ratio of population i.
    """
    ...
    for area in list(neuronal_densities.keys()):
        ...
        """
        supplement:compute the number of neurons in each layers
        """ 
        realistic_neuronal_numbers[area]['1']  = (S * ltc['1'] *nd['overall']) 
        realistic_neuronal_numbers[area]['23'] = (S * ltc['23'] *nd['23'])       
        realistic_neuronal_numbers[area]['4'] = (S * ltc['4'] *nd['4'])       
        realistic_neuronal_numbers[area]['5'] = (S * ltc['5'] *nd['5'])       
        realistic_neuronal_numbers[area]['6'] = (S * ltc['6'] *nd['6'])     
        realistic_neuronal_numbers[area]['total'] = sum(realistic_neuronal_numbers[area].values())
        
        num = {
            "23":num_V1['E23']['neurons']+num_V1['V23']['neurons']+\
            num_V1['P23']['neurons']+num_V1['S23']['neurons'],
            "4":num_V1['E4']['neurons']+num_V1['V4']['neurons']+\
            num_V1['P4']['neurons']+num_V1['S4']['neurons'],
            "5":num_V1['E5']['neurons']+num_V1['V5']['neurons']+\
            num_V1['P5']['neurons']+num_V1['S5']['neurons'],
            "6":num_V1['E6']['neurons']+num_V1['V6']['neurons']+\
            num_V1['P6']['neurons']+num_V1['S6']['neurons']
            }
        ...
        for layer_type in layer_types:
            if layer_type == "1":
                realistic_neuronal_numbers[area]["H1"] = realistic_neuronal_numbers[area]['1']
            else:
                for neuron_type in neuron_types:
                    realistic_neuronal_numbers[area][neuron_type+layer_type] = \
                    (realistic_neuronal_numbers[area][layer_type]* num_V1[neuron_type+layer_type]['neurons'] / (num[layer_type]))
