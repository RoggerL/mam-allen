...
def matrix_to_dict(m, area_list, structure, external=None):
    ...
    for area, area2 in product(area_list, area_list):
        ...
        if area == 'TH' and area2 == 'TH':
            x = x.reshape((len(structure[area]), len(structure[area2])))
            x = np.insert(x, 4, np.zeros((4, len(structure[area])), dtype=float), axis=0)
            x = np.insert(x, 4, np.zeros((4, len(structure[area])+4), dtype=float), axis=1)
        elif area2 == 'TH':
            x = x.reshape((len(structure[area]), len(structure[area2])))
            x = np.insert(x, 4, np.zeros((4, len(structure[area])), dtype=float), axis=1)
        elif area == 'TH':
            x = x.reshape((len(structure[area]), len(structure[area2])))
            x = np.insert(x, 4, np.zeros((4, len(structure[area2])), dtype=float), axis=0)
        else:
            x = x.reshape((len(structure[area]), len(structure[area2])))
...
#     """
#     tau_syn_E = neuron_params['E']['tau_syn']
#     C_m = neuron_params['C_m']
#     PSP_transform = tau_syn_E / C_m

#     return PSP_transform * W

def convert_syn_weight(W, area_list, structure, neuron_params):
    """
    Convert the amplitude of the PSC into mV.
    ...
    ----------
    W : dict
        dict of Synaptic weight defined as the amplitude of the post-synaptic current.
    area_list: list
        List of areas in the network. Defines the order of areas
        in the given matrix.
    structure : dict
    ...
    for target_area in area_list:
        J[target_area] = {}
        for target_pop in structure[target_area]:
            J[target_area][target_pop] = {}
            for source_area in area_list:
                J[target_area][target_pop][source_area] = {}
                for source_pop in structure[source_area]:
                    # J[target_area][target_pop][source_area][source_pop] = {}
                    if source_pop[0] == 'E':
                        J[target_area][target_pop][source_area][source_pop] = W[target_area][target_pop][source_area][source_pop]*neuron_params['E']['tau_syn']/neuron_params['E']['C_m']
                    if source_pop[0] == 'H':
                        J[target_area][target_pop][source_area][source_pop] = W[target_area][target_pop][source_area][source_pop]*neuron_params['H']['tau_syn']/neuron_params['H']['C_m']
                    if source_pop[0] == 'S':
                        J[target_area][target_pop][source_area][source_pop] = W[target_area][target_pop][source_area][source_pop]*neuron_params['S']['tau_syn']/neuron_params['S']['C_m']
                    if source_pop[0] == 'P':
                        J[target_area][target_pop][source_area][source_pop] = W[target_area][target_pop][source_area][source_pop]*neuron_params['P']['tau_syn']/neuron_params['P']['C_m']
                    if source_pop[0] == 'V':
                        J[target_area][target_pop][source_area][source_pop] = W[target_area][target_pop][source_area][source_pop]*neuron_params['V']['tau_syn']/neuron_params['V']['C_m']
