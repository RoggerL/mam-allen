...
def computer_current(g_dict,showmatrix=False):
    ...
    updated_g_dict = deepcopy(g_dict)
    beta = 1e-6
    ...
    for target_pop in pop_list_norm:
        ...
        for source_pop in pop_list_norm:
            ...
            currents[target_pop][source_pop] = K_in[target_pop][source_pop]*W_in[target_pop][source_pop]*0.5*rate
        ...
        for source_pop in pop_list_norm:
            if K_in[target_pop][source_pop] > 0:
                if source_pop[0] == "E":
                    print(source_pop)
                    updated_value = updated_g_dict[source_pop] - beta * currents[target_pop]["total"] / (K_in[target_pop][source_pop]*0.5*rate)
                else:
                    updated_value = updated_g_dict[source_pop] - beta * currents[target_pop]["total"] / (K_in[target_pop][source_pop]*0.5*rate*(-4.))

                if updated_value > 0:
                    updated_g_dict[source_pop] = updated_value
                else: 
                    updated_g_dict[source_pop] = 0    
...
for i in range(10):
    ...
    for i in range(1):
        g_dict = computer_current(g_dict)

    # print("g_dict=",g_dict)
    computer_current(g_dict,showmatrix=True)
...
input_update = copy.deepcopy(input_current)
