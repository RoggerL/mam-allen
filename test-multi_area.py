    ...
    with dataset.connect(f'sqlite:///{data_path}/dataset.db') as db:
        for input_pop in pop_list_norm:
            ...
            for param_name in pop_list_norm:
                ...
                # 遍历 k 值
                for k in range(10):
                    if len(list(db[input_pop+param_label].find(param_value=k))) > 0:
                        # value_k_list.append(k)
                        value_k = db[input_pop+param_label].find(param_value=k)
                        for row in value_k:
                            print("row=",row)
                            # if row[param_name] < 100:
                            if True:
                                value_k_list.append(k+1)
                                rate_k_list.append(row[param_name+"_"+area])
            ...
            plt.title("Variation of discharge rate of neural populations \n in V1 with relative synaptic strength of {}'s projection in {}".format(input_pop,area))
            plt.xlabel("Relative increment in synaptic strength of {}'s projection in {}: $(w-w_0)/w_0$".format(input_pop,area))
            plt.ylabel("Discharge rate of neural populations in {}: r/Hz".format(area))
            plt.legend(loc='upper left',fontsize='small',bbox_to_anchor=(0.99, 1))
            plt.savefig("/home/liugangqiang/fig4/rate with strength of {}'s projection in {}".format(input_pop,area))
            plt.close()